...
async def _verify_predictions(session: AsyncSession, fund_code: str, current_price: float, current_date: str):
    ...
    for pred_rec in preds:
        ...
        if updated:
            ...
            if verified:
                hit_rate = sum(1 for r in verified if r) / len(verified)
                pred_rec.confidence = round(min(0.95, max(0.1, hit_rate)), 2)
...
async def _update_evolver(session: AsyncSession, fund_code: str, rows: list):
    ...
    if len(all_snaps) >= 3:
        ...
        def _ema_trend(vals, window=3):
            """Son window snapshot'ın ağırlıklı ortalaması ile önceki window'u karşılaştır.
            Tek adımlık gürültüye dayanıklı EMA-bazlı trend tespiti."""
            if len(vals) < 2: return "belirsiz"
            w = min(window, len(vals))
            weights = list(range(1, w+1))  # [1, 2, 3] — yeni snapshot daha ağırlıklı
            recent = vals[-w:]
            recent_ema = sum(v*wt for v,wt in zip(recent, weights)) / sum(weights)
            if len(vals) < w+2:
                # Karşılaştırmak için yeterli önceki veri yok, tek adım kullan
                return "artıyor" if vals[-1] > vals[-2] else "azalıyor"
            prev = vals[-(w*2):-w]
            prev_ema = sum(v*wt for v,wt in zip(prev[-w:], weights)) / sum(weights)
            diff_pct = (recent_ema - prev_ema) / abs(prev_ema) * 100 if prev_ema != 0 else 0
            if diff_pct > 3: return "artıyor"
            if diff_pct < -3: return "azalıyor"
            return "yatay"

        # Sharpe trendi — EMA bazlı
        sharpes = [d.get("kazanc_risk_skoru") for d in snapdata if d.get("kazanc_risk_skoru") is not None]
        sharpe_trend = _ema_trend(sharpes)

        # Momentum trendi — EMA bazlı
        moms = [d.get("momentum_30d") for d in snapdata if d.get("momentum_30d") is not None]
        mom_trend_raw = _ema_trend(moms)
        mom_trend = "güçleniyor" if mom_trend_raw == "artıyor" else "zayıflıyor" if mom_trend_raw == "azalıyor" else "yatay"

        # Volatilite trendi — EMA bazlı
        vols = [d.get("annual_volatility") for d in snapdata if d.get("annual_volatility") is not None]
        vol_trend = _ema_trend(vols)

        # Drawdown trendi — EMA bazlı (artan dd = kötüleşiyor)
        dds = [d.get("max_drawdown") for d in snapdata if d.get("max_drawdown") is not None]
        dd_raw = _ema_trend(dds)
        dd_trend = "kötüleşiyor" if dd_raw == "artıyor" else "iyileşiyor" if dd_raw == "azalıyor" else "yatay"

        # ── Streak sayacı (üst üste pozitif/negatif gün) ──
        streak_positive = 0
        streak_negative = 0
        for r in reversed(all_returns):
            if r > 0:
                if streak_negative > 0: break
                streak_positive += 1
            else:
                if streak_positive > 0: break
                streak_negative += 1

        # ── Calmar ratio (risk-adjusted long-term perf) ──
        calmar = None
        if max_dd > 0:
            total_ret = round((prices[-1] - prices[0]) / prices[0] * 100, 2)
            trading_days = len(prices)
            annual_ret = total_ret * (252 / trading_days) if trading_days > 0 else 0
            calmar = round(annual_ret / max_dd, 2)

        # ── Regime tespiti ──
        if ann_vol > 40 and sharpe < 0 and max_dd > 20:
            regime = "kriz"
        elif ann_vol > 25 and sharpe < 0.5:
            regime = "zorlu"
        elif ann_vol < 15 and sharpe > 1.0:
            regime = "ideal"
        elif sharpe > 0.5 and mom_trend_raw == "artıyor":
            regime = "güçlü"
        else:
            regime = "normal"
        ...
        # Streak sinyalleri
        if streak_positive >= 5:
            signals.append({"type": "pozitif", "msg": f"{streak_positive} gün üst üste pozitif kapanış — güçlü kısa vadeli ivme"})
        elif streak_negative >= 5:
            signals.append({"type": "negatif", "msg": f"{streak_negative} gün üst üste negatif kapanış — dikkat, trend kötü"})

        # Calmar sinyali
        if calmar is not None:
            if calmar > 2.0:
                signals.append({"type": "pozitif", "msg": f"Risk-getiri dengesi mükemmel (Calmar {calmar:.2f}) — çekilen her birim riske karşı yüksek kazanç"})
            elif calmar < 0.5:
                signals.append({"type": "uyarı", "msg": f"Risk-getiri dengesi zayıf (Calmar {calmar:.2f}) — yüksek drawdown'a göre getiri düşük"})

        # Regime sinyali
        if regime == "kriz":
            signals.append({"type": "negatif", "msg": "Fon kriz modunda — yüksek volatilite, negatif kazanç skoru ve ciddi drawdown bir arada"})
        elif regime == "ideal":
            signals.append({"type": "pozitif", "msg": "Fon ideal rejimde — düşük volatilite, güçlü kazanç skoru"})
        elif regime == "güçlü":
            signals.append({"type": "pozitif", "msg": "Fon güçlü rejimde — pozitif kazanç skoru ve artan momentum"})
...
{{"fundCode":"{fund_code}","fundName":"...","company":"...","month":"Ocak 2026","monthKey":"2026-01","monthlyReturn":8.19,"yearlyReturn":74.24,"avgMaturity":39.98,"monthlyTurnover":1.65,"topHoldings":[{{"name":"AKBNK","fullName":"Akbank T.A.Ş.","weight":5.5}}],"expenses":{{"managementFee":1.75,"custodyFee":0.0917,"totalExpenseRatio":0.2208}},"riskScore":4,"stopajRate":17.5,"valor":"T+1/T+2","fundType":"Değişken Fon","portfolioItems":[{{"name":"Hisse Senedi","value":40.0,"category":"equity"}}],"aiInsights":["Tespit 1","Tespit 2","Tespit 3"],"dexterRecommendations":["TEFAS+evolver'a dayalı öneri 1","Öneri 2","Öneri 3"],"twitterSummary":"📊 URA | ATA PORTFÖY ENERJİ DEĞİŞKEN FON\n━━━━━━━━━━━━━━\n🚀 Aylık: +27.06% | Yıllık: +95.55%\n💼 Portföy: ₺939.8M | 12.832 yatırımcı\n⚠️ Risk: 5/7 · Orta-Yüksek\n\n🔍 Öne Çıkanlar:\n• %63 yabancı uranyum hisseleri\n• Güçlü momentum, hacim artışı\n• Enflasyona karşı alternatif varlık\n\n📅 Ocak 2026 · KAP Raporu\n#Enerji #Uranyum #KAP #YatırımFonu #TEFAS"}}"""
    ...
    try:
        ...
        analysis_dict = json.loads(text.strip().rstrip("```").strip())
    except Exception as e:
        print(f"JSON Parse Hatası: {e}")
        analysis_dict = {"aiInsights": "Analiz oluşturulamadı.", "dexterRecommendations": "Veri eksik."}

    # 3. Ek Bilgileri Ekle (Claude'a sormadan Python tarafında ekliyoruz)
    analysis_dict["scorecard"] = scorecard
    analysis_dict["fund_code"] = fund_code

    # 4. VERİTABANINA KAYIT (Cache için en kritik yer)
    try:
        today_str = datetime.now().strftime("%Y-%m-%d")
        new_record = FonAnaliz(
            code=fund_code,
            date=today_str,
...
def _calculate_scorecard(prices: list, monthly_return: float, yearly_return: float,
                          ...
                          mevduat_aylik: float = None, evolver_data: list = None) -> dict:
    if mevduat_aylik is None:
...
Regime: {evolver.get("regime","normal")} | Calmar: {evolver.get("calmar_ratio","?")} | Streak: {f'+{evolver.get("streak_positive",0)} gün pozitif' if evolver.get("streak_positive",0) > 2 else f'-{evolver.get("streak_negative",0)} gün negatif' if evolver.get("streak_negative",0) > 2 else "karışık"}
...
#YatırımFonu #TEFAS #Borsaİstanbul"""
    ...
    try:
        import os as _os
        _os.makedirs("/root/FONAR/dataset", exist_ok=True)
        dataset_entry = {
            "fund_code": fund_code,
...
Risk Skoru: {fund_info.get("riskScore","?")}/7""",
            "output": "\n".join(["• " + str(i) for i in ai.get("aiInsights",[])]) +
                      "\n\nÖneriler:\n" +
                      "\n".join(["• " + str(r) for r in ai.get("dexterRecommendations",[])]),
            "metadata": {
        ...
        with open("/root/FONAR/dataset/analyses.jsonl", "a", encoding="utf-8") as _f_ds:
            _f_ds.write(json.dumps(dataset_entry, ensure_ascii=False) + "\n")
...
async def analyze_tefas(fund_code: str):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if current_price and ai.get("dexterRecommendations"):
            ...
            # Skor tabanlı yön tespiti — her kelime ağırlık taşır
            _bear_words = {
                "sat": 3, "çık": 3, "çıkış": 3, "realizasyon": 3,
                "azalt": 2, "düşüş": 2, "düşecek": 2, "dikkat": 1,
                "risk": 1, "kaçın": 2, "olumsuz": 2, "kayıp": 2,
            }
            _bull_words = {
                "al": 3, "giriş": 3, "artış": 2, "yüksel": 2,
                "güçlen": 2, "fırsat": 2, "olumlu": 2, "büyü": 1,
                "tut": 1, "devam": 1,
            }
            # "bekle" hem bullish hem bearish olabilir — bağlama göre
            # "toparlanmayı bekle" → bearish bağlam, "fırsatı bekle" → bullish
            if "toparlanmayı bekle" in recs_text or "düzeltmeyi bekle" in recs_text:
                _bear_words["bekle"] = 2
            elif "fırsatı bekle" in recs_text or "girişi bekle" in recs_text:
                _bull_words["bekle"] = 1

            bear_score = sum(w for kw, w in _bear_words.items() if kw in recs_text)
            bull_score = sum(w for kw, w in _bull_words.items() if kw in recs_text)
...
async def get_top5():
    ...
    for code in codes:
        ...
        # Dexter yön tahmini bonusu — güvenilirliğe ve doğruluğa göre ölçekli
        dexter_bonus = 0
        try:
            ...
            if dp_mems:
                ...
                direction = dp.get("direction", "neutral")
                # Sinyal doğruluk oranını al
                try:
                    sa_raw = ev_by_type.get("signal_accuracy", "{}")
                    sa = json.loads(sa_raw) if sa_raw else {}
                    acc_pct = sa.get("accuracy_pct", 50)
                    total_pred = sa.get("total_predictions", 0)
                except:
                    acc_pct, total_pred = 50, 0
                # Güvenilirlik çarpanı: az tahminle tam güven yok
                reliability = min(1.0, total_pred / 10) if total_pred < 10 else 1.0
                # Doğruluk çarpanı: %50 = nötr, %80 = güçlü
                acc_factor = (acc_pct - 50) / 50  # -1 ile +1 arasında
                # Max bonus ±10, güvenilirlik ve doğrulukla ölçeklenir
                max_bonus = 10
                if direction == "bullish":
                    dexter_bonus = round(max_bonus * reliability * max(0.2, acc_factor), 1)
                elif direction == "bearish":
                    dexter_bonus = -round(max_bonus * reliability * max(0.2, abs(acc_factor)), 1)
        except: pass
